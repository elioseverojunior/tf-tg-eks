...
remote_state {
  ...
  config = {
    bucket         = "${yamldecode(file(find_in_parent_folders("global_values.yaml")))["s3_bucket_tf_state"]}"
    key            = "${path_relative_to_include()}/terraform.tfstate"
    region         = "${yamldecode(file(find_in_parent_folders("global_values.yaml")))["tf_state_bucket_region"]}"
    encrypt        = true
    dynamodb_table = "${yamldecode(file(find_in_parent_folders("global_values.yaml")))["s3_bucket_tf_state"]}-state-lock"
  }
...
#        "destroy",
#      ]
#      execute = ["find", ".", "-name", ".terragrunt-cache", "-type", "d", "-exec", "rm", "-rf", "{} \\;"]
#    }
# }
