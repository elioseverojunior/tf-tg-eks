...
inputs = {
  ...
  eks_managed_node_groups = {
    "${include.root.locals.full_name}-${include.root.locals.merged.aws_region}a" = {
      desired_size            = 1
      ami_type                = "AL2_x86_64"
      platform                = "linux"
    ...
    "${include.root.locals.full_name}-${include.root.locals.merged.aws_region}b" = {
    ...
    "${include.root.locals.full_name}-${include.root.locals.merged.aws_region}c" = {
      ami_type                   = "BOTTLEROCKET_x86_64"
      platform                   = "bottlerocket"
      instance_types             = ["t3a.large"]
